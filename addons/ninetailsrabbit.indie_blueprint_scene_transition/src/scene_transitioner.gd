class_name SceneTransitioner extends Node
	...
	if is_instance_valid(current_transition):
		...
		match load_status:
			...
			ResourceLoader.THREAD_LOAD_FAILED:
				push_error("IndieBlueprintSceneTransitioner: An error %s happened in the process of loading the scene %s, aborting the transition..." %[error_string(load_status), current_transition.next_scene_path] )
				load_finished.emit(current_transition)
				current_transition = null
				
			ResourceLoader.THREAD_LOAD_INVALID_RESOURCE:
				push_error("IndieBlueprintSceneTransitioner: An error %s happened, the scene %s is invalid, aborting the transition..." %[error_string(load_status), current_transition.next_scene_path] )
				load_finished.emit(current_transition)
				current_transition = null


func transition_to(scene: Variant) -> SceneTransitioner:
	if current_transition == null:
	...
	if scene is PackedScene:
		...
		current_transition = null
		push_error("IndieBlueprintSceneTransitioner: The scene %s to transition is not a valid PackedScene or path" % scene)
	
	return self


func use_subthreads(enable: bool = false) -> SceneTransitioner:
	if current_transition == null:
		current_transition = CurrentTransition.new()
...
func in_transition(transition_id: StringName, args: Dictionary = {}) -> SceneTransitioner:
	var transition: IndieBlueprintSceneTransitionConfiguration = get_transition_by_id(transition_id)
	
	if transition:
		if current_transition == null:
			current_transition = CurrentTransition.new()
		
		current_transition.transition_in["transition"] = transition.scene.instantiate() as IndieBlueprintSceneTransition
...
func out_transition(transition_id: StringName, args: Dictionary = {}) -> SceneTransitioner:
	var transition: IndieBlueprintSceneTransitionConfiguration = get_transition_by_id(transition_id)
	...
	if transition:
		...
		current_transition.transition_out["transition"] = transition.scene.instantiate() as IndieBlueprintSceneTransition
