class_name WarpLoadingScreen extends Control
...
func _ready() -> void:
	...
	if load_error != OK:
		push_error("WarpLoadingScreen: An error %s happened when trying to load the scene %s " % [error_string(load_error), next_scene_path])
	...
	if loading:
		...
		match scene_load_status:
			...
			ResourceLoader.THREAD_LOAD_FAILED, ResourceLoader.THREAD_LOAD_INVALID_RESOURCE:
				push_error("WarpLoadingScreen: The resource load failed with status %s " % scene_load_status)
				loading = false
